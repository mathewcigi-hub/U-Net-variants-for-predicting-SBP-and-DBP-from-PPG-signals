...
from U_net_modified import unet_1d

# Define the data generator class
class HDF5DataGenerator(tf.keras.utils.Sequence):
    def __init__(self, folder, file_list, batch_size, input_size, shuffle=True):
        self.folder = folder
        ...
        # Calculate the total number of samples and indexes
        for file in self.file_list:
            file_path = os.path.join(self.folder, file)
            with h5py.File(file_path, 'r') as hdf5_file:
                file_samples = hdf5_file['PPG_values'].shape[0]
                self.indexes.extend([(file, i) for i in range(file_samples)])
    ...
    def __data_generation(self, batch_indexes):
        ...
        for i, (file, idx) in enumerate(batch_indexes):
            file_path = os.path.join(self.folder, file)
            with h5py.File(file_path, 'r') as hdf5_file:
                X_batch[i, :, 0] = hdf5_file['PPG_values'][idx]
                bp_class_str = hdf5_file['BP_Classification'][idx].decode('utf-8')
                bp_class_num = self.bp_class_index[bp_class_str]
                y_batch[i] = tf.keras.utils.to_categorical(bp_class_num, num_classes=6)
...
if __name__ == "__main__":
    ...
    folder = r'H:\HTIC\PulseDB\For_25k_data\PulseDB_25k_PPG_F_details\VitalDB_json_files_new\Files_npy_hdf5\Hdf5_files'
    batch_size = 32
    input_size = (256, 1)
    epochs = 100
    final_model_path = r'H:\HTIC\PulseDB\For_25k_data\PulseDB_25k_PPG_F_details\VitalDB_json_files_new\Files_npy_hdf5\combined_model.h5'

    # Load list of HDF5 files and split into train/validation sets
    file_list = [f for f in os.listdir(folder) if f.endswith('.hdf5')]
    train_files, val_files = train_test_split(file_list, test_size=0.2, random_state=42)

    # Create data generators
    train_gen = HDF5DataGenerator(folder, train_files, batch_size, input_size)
    val_gen = HDF5DataGenerator(folder, val_files, batch_size, input_size, shuffle=False)
